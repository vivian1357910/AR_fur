...
public class InteractionController : ARBaseGestureInteractable
{
    ...
    [SerializeField]
        ...
        void CrosshairCalculation()
        {
            ...
            if (_raycastManager.Raycast(origin, s_Hits, TrackableType.PlaneWithinPolygon))
            {
                pose = s_Hits[0].pose;
        ...
        protected override void OnEndManipulation(TapGesture gesture)
        {
            ...
            if (IsPointerOverUI(gesture))
                return;
            if (_raycastManager.Raycast(gesture.startPosition, s_Hits, TrackableType.PlaneWithinPolygon))
            {
                var hit = s_Hits[0];
